import { Plus, ListChecks, Search, ClipboardList, Users, PieChart } from "lucide-react";
...
const donorActions: ActionConfig[] = [
    ...
    },
    {
        ...
        path: "/donations", // Corrected path from /listings to /donations based on routing
        variant: "secondary",
    },
];
...
export function QuickActions({ role }: { role: UserRole }) {
    ...
    const actions = role === "donor" ? donorActions : role === "ngo" ? ngoActions : adminActions;

    return (
        <div className="bg-white/70 backdrop-blur-lg border border-white/20 shadow-sm rounded-xl p-5 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {actions.map((action) => (
                    <button
                        ...
                        className={`flex items-center gap-4 p-4 rounded-xl border transition-all duration-200 text-left group ${action.variant === "primary"
                    ...
                    >
                        <div
                            className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${action.variant === "primary"
                        ...
                        <div>
                            ...
                            <div
                                className={`text-xs mt-0.5 ${action.variant === "primary" ? "text-primary-foreground/80" : "text-muted-foreground"
                                    }`}
                            >
                                {action.description}
                            </div>
